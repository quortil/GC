...
##
m <- GC.activity.metrics(compare=FALSE)
...
plotRGC <- function (Pstart = 350, Pend = 220, PPstart = 61, PPend = 120, Pedal = "Left")
{
  ...
  # Postion labels
  labelPosition(Pstart%%360, 0.7) 
  labelPosition(Pend, 0.7) 
  labelPosition(PPstart%%360, 0.45) 
  labelPosition(PPend, 0.45) 
}

par(mfrow=c(1,2), mar=c(0,0,0,0))

plotRGC(m$Average_Left_Power_Phase_Start, m$Average_Left_Power_Phase_End,m$Average_Left_Peak_Power_Phase_Start,m$Average_Left_Peak_Power_Phase_End,Pedal = "Left")
plotRGC(m$Average_Right_Power_Phase_Start, m$Average_Right_Power_Phase_End,m$Average_Right_Peak_Power_Phase_Start,m$Average_Right_Peak_Power_Phase_End,Pedal = "Right")
